function AddNewNews()
{
 ...
 var ajxNews= {
		listNews:function(spage){
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"listNews",pg:spage,kw:$("#txtKeyword").val(),type:$("#sltCategory").val(),typeDetail:$("#sltCategoryDetail").val(),iduser:$("#sltUser").val()},
				function(data){
				if(data!="")
				{
			        $('#ListTD').html(data);
		...
		ChangeStatus:function(){
				...
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"ChStatusNews",status:$("#sltStatus").val(),id:chked},
				function(){
				ajxNews.listNews(1);
			});
		},
		Delete:function(idtd){
		        if (!confirm('Bạn có chắc chắn muốn xóa tin tức này không?'))
                    return false;
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"deleteNews",id:idtd},
				function(){
				ajxNews.listNews(1);
			});
		},
		DeleteImage:function(idtr,image,idtd){
		    $.post(PathAdmin +"/Handler/Admin.aspx", { act:"deleteImgNews",id:idtd,img:image},
		    function(){
		     $("#"+idtr).hide();
			});
		},
