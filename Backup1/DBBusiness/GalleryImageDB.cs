...
namespace DBBusiness
{
    public class GalleryImageDB
    {
        ...
        {
            return db.MyExecuteNonQuery("insert into GalleryImage (Photo,Description) values(@Photo,@Description)", CommandType.Text, ref err,
                new SqlParameter("@Photo", td.Photo),
                new SqlParameter("@Description", td.Description)
                );
        }
