...
function changeSelect(se,hd)
{
 ...
 var ajxServices= {
		...
		$.post(PathAdmin + "/Handler/Admin.aspx", { act: "listServices", pg: spage, kw: $("#txtKeyword").val(), type: $("#sltCategory").val(), typeDetail: $("#sltCategoryDetail").val() },
				function(data){
				if(data!="")
				{
			        $('#ListTD').html(data);
		...
		ChangeStatus:function(){
                ...
                $.post(PathAdmin + "/Handler/Admin.aspx", { act: "ChStatusServices", status: $("#sltStatus").val(), id: chked },
				function(){
				ajxServices.listProducts(1);
			});
		},
		Delete:function(idtd){
		if (!confirm('Bạn có chắc chắn muốn xóa dịch vụ này không?'))
                    return false;
                $.post(PathAdmin + "/Handler/Admin.aspx", { act: "deleteServices", id: idtd },
				function(){
				ajxServices.listProducts(1);
			});
		},
		DeleteImage:function(idtr,image,idtd){
		$.post(PathAdmin + "/Handler/Admin.aspx", { act: "deleteImgServices", id: idtd, img: image },
		    function(){
		     $("#"+idtr).hide();
			});
		},
		DeleteFile: function(idtr, filename, id) {
		$.post(PathAdmin + "/Handler/Admin.aspx", { act: "deletefileServices", filename: filename, id: id },
		    function(){
		     $("#"+idtr).hide();
			});
		},
		GetListCatSubServices: function() {
				document.getElementById("hdCategoryDetail").value="";
				$.post(PathAdmin + "/Handler/Admin.aspx", { act: "GetListCatSubServices", cat: $("#sltCategory").val() },
			    function(data){
			    if(data.XMLDocument!="")
			    {
...
{			
    pg = numberpage;
    ajxServices.listProducts(numberpage);
}
