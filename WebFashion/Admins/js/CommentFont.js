...
function changeSelect(se,hd)
{
 ...
 var ajxComment= {
		listcomment:function(spage){
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"listcommentfont",pg:spage,kw:$("#txtKeyword").val()},
				function(data){
				if(data!="")
				{
			        $('#ListTD').html(data);
		...
		ChangeStatus:function(){
				...
				if(chked=='')
				{
				    alert("Bạn phải chọn bài bình luận!");
				    return false;
				}
				 if($("#sltStatus").val()=="")
                {
				...
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"ChStatusCommentFont",status:$("#sltStatus").val(),id:chked},
				function(){
				ajxComment.listcomment(1);
			});
		},
		...
		Delete:function(idtd){
		        if (!confirm('Bạn có chắc chắn muốn xóa bài bình luận này không?'))
                    return false;
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"deleteCommentFont",id:idtd},
				function(){
				ajxComment.listcomment(1);
			});
		},
...
{			
    pg = numberpage;
    ajxComment.listcomment(numberpage);
}
