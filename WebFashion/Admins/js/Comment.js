...
function changeSelect(se,hd)
{
 ...
 var ajxComment= {
		listcomment:function(spage){
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"listcomment",pg:spage,kw:$("#txtKeyword").val()},
				function(data){
				if(data!="")
				{
			        $('#ListTD').html(data);
		...
		ChangeStatus:function(){
				...
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"ChStatusComment",status:$("#sltStatus").val(),id:chked},
				function(){
		...
		Delete:function(idtd){
		        if (!confirm('Bạn có chắc chắn muốn xóa bài bình luận này không?'))
                    return false;
				$.post(PathAdmin +"/Handler/Admin.aspx", { act:"deleteComment",id:idtd},
				function(){
				ajxComment.listcomment(1);
			});
		},
